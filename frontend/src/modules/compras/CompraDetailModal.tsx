...
export const CompraDetailModal = ({ compraId, onClose }: CompraDetailModalProps) => {
  ...
  useEffect(() => {
    if (compraId) {
      setLoading(true);
      api.get(`/compras/${compraId}`).then(res => {
    ...
    <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-3xl overflow-hidden shadow-xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <h2 className="text-xl font-bold text-gray-800">Detalle de Compra</h2>
          ...
          {loading ? (
             ...
             <div className="space-y-6">
               <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                 <div>
                   <p className="text-xs text-gray-500 uppercase font-semibold">Nº Doc.</p>
                   <p className="font-bold text-gray-900">{data.NumeroDocumento || '-'}</p>
                 </div>
                 <div>
                 ...
                 {data.Observacion && (
                   <div className="col-span-2 md:col-span-4">
                     ...
                     <p className="text-gray-700 text-sm mt-1">{data.Observacion}</p>
                   </div>
                 )}
               </div>
               
               <div>
                 <h3 className="text-lg font-bold mb-4 text-gray-800">Productos Registrados</h3>
                 <div className="border border-gray-100 rounded-xl overflow-hidden">
                   <table className="w-full text-left text-sm">
                     ...
                     <tfoot className="bg-gray-50/50">
                       <tr>
                         <td colSpan={3} className="px-4 py-3 text-right font-bold text-gray-900 text-base">Total General:</td>
