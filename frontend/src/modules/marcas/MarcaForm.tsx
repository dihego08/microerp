...
export const MarcaForm = ({ itemId, onClose }: { itemId: number | null, onClose: () => void }) => {
  const isEditing = !!itemId;
  const queryClient = useQueryClient();
  const { register, handleSubmit, reset } = useForm();

  const { data: item, isLoading } = useQuery({
    queryKey: ['marcas', itemId],
    queryFn: async () => {
      const res = await api.get(`/marcas/${itemId}`);
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => {
      ...
      if (isEditing) return api.put(`/marcas/${itemId}`, data);
